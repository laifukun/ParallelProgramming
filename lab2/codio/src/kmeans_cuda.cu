...
__global__ void group_centroid_shared(DataType* points, DataType* centroids,  int* cluster_of_points, int* cluster_size, int n_points, int n_centroids, int dims) {
    ...
    DataType* block_cluster_point_sum = (DataType*)&localMemory[TPB*dims*sizeof(DataType)];
    int* block_cluster_size = (int*)&localMemory[TPB*dims*sizeof(DataType) + n_centroids*dims*sizeof(DataType)];
    
    for (int i = threadId; i < n_centroids; i += TPB) {
        for (int d = 0; d < dims; d++)
    ...
    for (int d = 0; d < dims; d++) {                
          atomicAdd(&block_cluster_point_sum[clusterId*dims + d], block_points[threadId*dims + d]);                
    }
    atomicAdd(&block_cluster_size[clusterId], 1);
    ...
    while (args->iter_converge < args->max_num_iter && !isConverged(args->centroids, tmpCentroid, args->threshold, n_centroids, dims)) {
       ...
       // }        
        ...
        for (int clusterId = 0; clusterId < args->num_cluster; clusterId++){
            printf("%d ", clusterId);
            for (int d = 0; d < args->dims; d++)
                printf("%lf ", tmpCentroid[clusterId*args->dims + d]);
        printf("\n");
        }
        */
    }
